...
app.get("/view", (req, res) => {
    let qry = "select * from test,media ";
...
app.get("/addmedia", (req, res) => {
      // fetching data from form
    const { vedio, name, image } = req.query

    
    let qry = "select * from media where images=? or vedios=?";
    mysql.query(qry, [vedio, name, image], (err, results) => {
        if (err)
            throw err
        else {

            if (results.length > 0) {
                res.render("media", { checkmesg: true })
            } else {

                
                let qry2 = "insert into media values(?,?,?)";
                mysql.query(qry2, [vedio, name,image], (err, results) => {
                    if (results.affectedRows > 0) {
                        res.render("media", { mesg: true })
                    }
                })
            }
        }
